...
import { getMotionVideoById, canUserAccessDevice } from './db';
...
videoRouter.get('/:videoId', async (req, res) => {
  try {
    ...
    // Verify user has access to the device (check session)
    ...
    const hasAccess = await canUserAccessDevice(user.id, video.deviceId);
    if (!hasAccess) {
