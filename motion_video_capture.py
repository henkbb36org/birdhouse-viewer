...
def get_device_info(device_id):
    ...
    try:
        ...
        cursor.execute(
            "SELECT id, ownerId, name FROM devices WHERE deviceId = %s",
            (device_id,)
        )
...
def finalize_video(device_id):
    ...
    if len(frames) < 5:  # Need at least a few frames
        ...
        mqtt_client.publish(f"device/{device_id}/control", "stop")
        logger.info(f"Sent stop command to {device_id}")
        # Unsubscribe from stream
        stream_topic = f"device/{device_id}/stream"
        mqtt_client.unsubscribe(stream_topic)
        logger.info(f"Unsubscribed from {stream_topic}")
        del active_captures[device_id]
        return
    
    # Generate filename
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    filename = f"{device_id}_{timestamp}.mp4"
    filepath = os.path.join(VIDEO_DIR, filename)
    
    # Create MP4 video
    if create_mp4_video(frames, filepath):
        # Get file size
    ...
    # Send stop command to ESP32-CAM to stop streaming
    mqtt_client.publish(f"device/{device_id}/control", "stop")
    logger.info(f"Sent stop command to {device_id}")
    
    # Unsubscribe from stream to stop receiving images
    stream_topic = f"device/{device_id}/stream"
    mqtt_client.unsubscribe(stream_topic)
    logger.info(f"Unsubscribed from {stream_topic}")
...
def start_video_capture(device_id):
    ...
    stream_topic = f"device/{device_id}/stream"
    mqtt_client.subscribe(stream_topic)
    logger.info(f"Subscribed to {stream_topic}")
...
def on_connect(client, userdata, flags, rc):
    ...
    if rc == 0:
        ...
        # Stream subscriptions are managed per-device when motion is detected
    else:
        ...
        if topic_type == 'motion':
            ...
            try:
                ...
                try:
                    ...
                    # If JSON, just check if message exists (any motion event)
                    logger.info(f"Motion detected on {device_id}")
                    start_video_capture(device_id)
                except json.JSONDecodeError:
                    # If not JSON, check for 'detected' string
                    if payload == 'detected':
                        logger.info(f"Motion detected on {device_id}")
                        start_video_capture(device_id)
            except Exception as e:
